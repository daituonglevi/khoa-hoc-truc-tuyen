...
-- First, ensure Courses table has primary key
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = 'Courses' AND CONSTRAINT_TYPE = 'PRIMARY KEY')
BEGIN
    ALTER TABLE [dbo].[Courses] ADD PRIMARY KEY (Id);
    PRINT 'Added PRIMARY KEY to Courses table';
END
GO

-- Add LiveClassId column to Lessons table (if not exists)
IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_NAME = 'Lessons' AND COLUMN_NAME = 'LiveClassId'
)
BEGIN
    ALTER TABLE [dbo].[Lessons] ADD [LiveClassId] [int] NULL;
    PRINT 'Added LiveClassId column to Lessons table';
END
ELSE
BEGIN
    PRINT 'LiveClassId column already exists in Lessons table';
END
GO
